...
function RoomType({
  ...
  let roomTypeIds = listTypeOfRooms?.map((i) => i.id);
    ...
    if (!e?.target?.checked) {
      setState({
        selectedItem: state.selectedItem.filter(
          (item, index) => !roomTypeIds.includes(item)
        ),
      });
    } else {
      setState({
        selectedItem: [...state.selectedItem, ...roomTypeIds].filter(
  ...
  //   setState({
  //     viewCurrentItem: listTypeOfRooms?.find(item => item.key == key),
...
export default connect(
  (state) => ({
    page: state.roomType.page || 0,
    size: state.roomType.size || 10,
    total: state.roomType.totalElements || 0,
